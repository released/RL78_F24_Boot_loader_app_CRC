...
enum
{
    RAM_FLAG_INDICATE_BOOT_APP ,    // 0xAA : boot , 0x55 : app
    RAM_FLAG_RESERVE2 ,
    RAM_FLAG_RESERVE3 ,
    RAM_FLAG_RESERVE4 ,
    RAM_FLAG_RESERVE5 ,
...
void ram_flag_test(void)
{
    unsigned char i = 0;

    for(i = 8 ; i < 16 ; i++)
    {
        _no_init_global[i] = 0x10 + i;
    }

    printf_tiny("ram_flag(1):0x%X",reset_to_bootloader>>16);
...
void hardware_init(void)
{
    ...
    _no_init_global[RAM_FLAG_INDICATE_BOOT_APP] = 0x55;  
